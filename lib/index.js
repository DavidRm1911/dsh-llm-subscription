 ...
 * plus a local Ollama option ($0, no login needed at all). Same seam as
 * @deepseek-ai/dsh-llm-deepseek (registerAdapter, LlmAdapter), confirmed by
 * reading that package's compiled source directly. Reasoning effort is
 * exposed via LlmModelReasoningInfo — the *existing* conversation.input.model
 * composer seat already renders whatever an adapter declares there, so this
 * needed no new client-ui plugin, just the right shape on
 * resolveModel()/prepareCall().
 *
 * No streaming deltas for claude/gemini (the underlying CLIs don't expose
 * token-by-token output in headless mode — a whole response arrives as one
 * text-delta); Ollama's own API does support streaming, not used here to
 * keep all three providers behaving the same way. No image support, no
 * credential handling (there's no credential to handle for any of these).
 */
...
const CLI_FOR_PROVIDER = { claude: "claude", gemini: "agy", "qwen-local": "ollama" };
...
const REASONING = {
	...
	// duplicate that choice. Ollama has no comparable effort concept. Neither
	// gets a `reasoning` entry.
};
...
const MODELS = {
	...
	"qwen-local": [{ id: "qwen-local", name: `Qwen (${OLLAMA_MODEL}, local)` }],
};
...
async function ollamaIsUp() {
	try {
		const res = await fetch(`${OLLAMA_HOST}/api/version`, { signal: AbortSignal.timeout(1000) });
		return res.ok;
	} catch {
		return false;
	}
...
// Lazy by design, same as the other two providers being absent from
// AVAILABLE when their CLI isn't installed: never starts `ollama serve` at
// module load or plugin apply time, only the first time a call actually
// needs it. The Mac coordinates agents, it doesn't become a permanent
// inference server just because the plugin is installed.
async function ensureOllamaServer() {
	if (await ollamaIsUp()) return;
	const bin = which("ollama");
	if (!bin) {
		throw new LlmError("llm-subscription: `ollama` is not on PATH to start it", "MISSING_CLI");
	}
	...
	throw new LlmError("llm-subscription: started `ollama serve` but it did not come up in time", "TIMEOUT");
}

class SubscriptionAdapter extends LlmAdapter {
	providerInfo(provider) {
		return { id: provider, name: PROVIDER_DISPLAY_NAME[provider] ?? provider };
	}
	...
	async *stream(options) {
		...
		if (options.provider === "claude") {
			result = await this.runClaude(prompt, options.system, effort, options.model);
		} else if (options.provider === "gemini") {
			result = await this.runGemini(prompt, options.system, options.model);
		} else {
			result = await this.runOllama(options.messages, options.system);
		}
	...
	async runOllama(messages, system) {
		...
		try {
			...
			throw new LlmError("llm-subscription: Ollama request failed", "PROVIDER_ERROR", { cause });
		}
		if (!res.ok) {
			const body = await res.text().catch(() => "");
			throw new LlmError(`llm-subscription: Ollama returned ${res.status}: ${body.slice(0, 200)}`, "PROVIDER_ERROR");
		}
		const data = await res.json();
		if (data.error) throw new LlmError(`Ollama reported an error: ${data.error}`, "PROVIDER_ERROR");
		return {
...
// Exported for the smoke test (test/smoke.mjs), which checks MODELS/
// CLAUDE_MODEL_FLAG/GEMINI_MODEL_FLAG stay in sync with each other —
// otherwise harmless to export.
export { MODELS, CLAUDE_MODEL_FLAG, GEMINI_MODEL_FLAG };
...
function apply(ctx) {
	...
	if (providers.length === 0) {
		ctx.logger?.warn?.(
			"llm-subscription: none of `claude`, `agy`, `ollama` found on PATH at startup — no providers registered",
		);
		return;
	}
	...
	for (const provider of providers) {
		ctx.llm.registerConfigurableProviders([
			{
				provider,
				displayName: PROVIDER_DISPLAY_NAME[provider] ?? provider,
